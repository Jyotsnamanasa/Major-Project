...
def add_FingerPrint():
    '''filename1 = filedialog.askopenfilename(initialdir = "/",
                                          ...
                                          filetypes = (("Text files",
                                                       ...
                                                       ("all files",
                                                        "*.*")),parent=newWindow)'''
    p=subprocess.Popen("C:\\Program Files\\Mantra\\MFS100\\Driver\\MFS100Test\\MANTRA.MFS100.Test.exe",cwd="C:\\Program Files\\Mantra\\MFS100\\Driver\\MFS100Test")
    p.wait()
    path1=f'fingerprint/{Roll_No_var.get()}.BMP'
    filename1="C:\\Program Files\\Mantra\\MFS100\\Driver\\MFS100Test\\FingerData\\FingerImage.bmp"
    ...
    if(Roll_No_var.get()=="" or name_var.get()=="" or branch.get()=="" or year_var.get()=="" or email_var.get()=="" or gender_var.get()=="" or contact_var.get()==""  ): 
        ...
        if len(p)>0 :
            ...
            if Roll_No_var.get() in name:
                string=Roll_No_var.get()+" is already available"
                messagebox.showerror("Error",string,parent=newWindow)
            else:
                flag=1
        else:
            flag=1
...
def get_update():
    date1=date.today()
    print(date1)
    d=list(map(int,str(date1).split("-")))
    con2=connect("holiday.db")
    cur2=con2.cursor()
    ...
    cur=con.cursor()
    date1=date.today()
    con1=connect("student.db")
    cur1=con1.cursor()
    cur1.execute("select RollNumber,Name,Branch,Year,email from student")
    rows=cur1.fetchall()
...
def detect():
        get_update()
        if status1=='Holiday':
            speak("No attendance will be taken today")
        else:
            ...
            # print(name)
                    ...
                    if roll_no!='Unknown':
                        ...
                        fingerprint_database_image=cv2.imread(f'E:\\project\\fingerprint\\{roll_no}.BMP')
                        ...
                        if best_score>90:
                            ...
                            if len(p)==0 and str(time1)>'08:00':
                                ...
                                speak("Thank you!")
                                time.sleep(5)
                            elif len(p)==1 and str(time1)>"13:00":
                                cur1.execute(f"select time_in from attendance where RollNumber=\'{roll_no}\' and day=\'{date1}\' ")
                                t1=cur1.fetchall()
                                #print(t1)
                                ...
                                time.sleep(15)
                            if str(time1)>'17:30':
                                cur1.execute(f"select RollNumber from attendance where day=\'{date1}\' and time_in='-")
                                p=cur1.fetchall()
                                status='Absent'
                                for row in p:
                                    cur1.execute(f"update attendance set status=\'{status}\' where RollNumber=\'{row[0]}\' and day=\'{date1}\'")
...
def get_frame():
    ...
    if check_var.get()=="Day wise":
        ...
        scroll_x=Scrollbar(Table_Frame,orient=HORIZONTAL)
        scroll_y=Scrollbar(Table_Frame,orient=VERTICAL)
        Attendance_table=ttk.Treeview(Table_Frame,columns=("RollNumber","Name","Branch","Year","Email","Day","Time_in","Time_out","status"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        ...
        Attendance_table.heading("Time_in",anchor=CENTER,text="Time_in")
        Attendance_table.heading("Time_out",anchor=CENTER,text="Time_out")
        ...
        Attendance_table.column("Time_in",anchor=CENTER,width=150)
        Attendance_table.column("Time_out",anchor=CENTER,width=150)
        ...
        global Attendance_table_month
        Table_Frame=Frame(Details_Frame,bd=4,relief=RIDGE,bg="mint cream")
        Table_Frame.place(x=10,y=120,width=1000,height=530)
        scroll_x=Scrollbar(Table_Frame,orient=HORIZONTAL)
        scroll_y=Scrollbar(Table_Frame,orient=VERTICAL)
        Attendance_table_month=ttk.Treeview(Table_Frame,columns=("RollNo","Name","Branch","Year","Email","Attendance"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=Attendance_table_month.xview)
        scroll_y.config(command=Attendance_table_month.yview)
        Attendance_table_month.heading("RollNo",anchor=CENTER,text="RollNo")
        Attendance_table_month.heading("Name",anchor=CENTER,text="Name")
        Attendance_table_month.heading("Branch",anchor=CENTER,text="Branch")
        Attendance_table_month.heading("Year",anchor=CENTER,text="Year")
        Attendance_table_month.heading("Email",anchor=CENTER,text="Email")
        Attendance_table_month.heading("Attendance",anchor=CENTER,text="Attendance")

        
        Attendance_table_month['show']="headings"
        Attendance_table_month.column("RollNo",anchor=CENTER,width=150)
        Attendance_table_month.column("Name",anchor=CENTER,width=150)
        Attendance_table_month.column("Branch",anchor=CENTER,width=150)
        Attendance_table_month.column("Year",anchor=CENTER,width=150)
        Attendance_table_month.column("Email",anchor=CENTER,width=150)
        Attendance_table_month.column("Attendance",anchor=CENTER,width=100)

        
        Attendance_table_month.pack(fill=BOTH,expand=1)
...
def get_report():
    if check_var.get()=="Day wise": 
        ...
        string1=f"SELECT RollNumber,Name,Branch,Year,mailid,day,time_in,time_out,status FROM attendance where day=\'{string2}\'"
        cur.execute(string1)
        rows=cur.fetchall()
        print(rows)
        ...
        if len(rows)!=0:
            Attendance_table_month.delete(*Attendance_table_month.get_children())
            for row in rows:
                Attendance_table_month.insert('',END,values=row)
            conn.commit()
        conn.close()
def get_csv_file():
    get_report()
    if check_var.get()=="Day wise":
        file1=date_var.get()+".csv"
        conn=connect("attendance.db")
        cur=conn.cursor()
        string2=date_var.get()
        string1=f"SELECT RollNumber,Name,Branch,Year,mailid,day,time_in,time_out,status FROM attendance where day=\'{string2}\'"
        ...
        conn=connect("attendance_month.db")
        cur=conn.cursor()
        string1=f"SELECT RollNumber,Name,Branch,Year,mailid,{month_var.get()} FROM attendance_month"
        cur.execute(string1)
        rows=cur.fetchall()
...
con1=connect('attendance.db')
cur1=con1.cursor()
cur1.execute("CREATE TABLE IF NOT EXISTS attendance(RollNumber text ,Name text NOT NULL,Branch text NOT NULL,Year text NOT NULL, mailid text NOT NULL,Day date NOT NULL,time_in timestamp not null,time_out timestamp not null,status text not null);")
con1.commit()
con1.close()
